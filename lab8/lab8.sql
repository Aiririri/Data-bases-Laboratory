-- 1. Drop tables if they exist to start fresh
...
-- Exercise 2.1: Create Simple B-tree Index
...
-- Exercise 2.2: Create Index on Foreign Key
...
-- Exercise 3.1: Create Multicolumn Index
...
-- Exercise 3.2: Create Multicolumn Index with Reversed Order
CREATE INDEX emp_salary_dept_idx ON employees(salary, dept_id);

-- Query 1: Filters by dept_id first (uses emp_dept_salary_idx)
SELECT * FROM employees WHERE dept_id = 102 AND salary > 50000;

-- Query 2: Filters by salary first (uses emp_salary_dept_idx)
SELECT * FROM employees WHERE salary > 50000 AND dept_id = 102;



-- Exercise 4.1: Create Unique Index
...
-- Test uniqueness (THIS QUERY WILL FAIL - EXPECT AN ERROR)
INSERT INTO employees (emp_id, emp_name, dept_id, salary, email)
VALUES (6, 'New Employee', 101, 55000, 'john.smith@company.com');

-- Exercise 4.2: Unique Constraint vs Index
-- Add phone column with constraint
ALTER TABLE employees ADD COLUMN phone VARCHAR(20) UNIQUE;

-- View the indexes to see the automatically created one
SELECT indexname, indexdef 
FROM pg_indexes 
...
-- Exercise 5.1: Create Index for Sorting (Descending)
...
-- Exercise 6.1: Function-Based Index (Case Insensitive)
...
-- Create index on the extracted year
...
-- Exercise 7.2: Drop Unused Index
...
-- Creating the specific filter index mentioned
...
-- Equality search (Uses Hash or B-tree)
SELECT * FROM projects WHERE proj_name = 'Website Redesign';

-- Range search (Uses B-tree only, Hash cannot do ranges)
...
-- Exercise 10.1: Review All Indexes and Sizes
SELECT 
    schemaname, 
    tablename, 
    indexname, 
